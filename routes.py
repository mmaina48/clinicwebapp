...
def Productdata(product):
    data=PurchaseItems.query.filter_by(product_name=product).all()
    dataArray=[]
    for product in data:
        productobj={}
...
def processinvoicedata():
    ...
    newinvoice=Order(customer_name=patient_name,created_on=invoiceDate_object,payment_type=paytype,\
        payment_amount=paid_amount,total_amount=grand_total_price,previous=previous,net_total=nettotal,total_due=due_amount,customer=patientToAdd)
   
    for (prod,prodtyp,qty,price,total) in zip(product_list,product_type,quantityarray,price,total_price):
        
        product_name=Product.query.filter_by(product_name=prod).one()

        productToreduce=PurchaseItems.query.filter_by(product_name=prod).first()
...
def ProductSale(product):
    productsold=Product.query.filter_by(product_name=product).one()
    productsale=OrderItems.query.filter_by(product_name=product).all()
    return render_template('productsales.html',productsale=productsale,productsold=productsold,product=product)
...
def processdata():
    ...
    newpurchase=Purchase(purchase_date=purchasedate_object,supplier_name=suplier_name,invoice_no=invoiceNo,\
        payment_type=paytype,total_amount=grand_total_price,payment_amount=paid_amount,supplier=supplierToAdd)
...
def MedicationSalesReport():
    sales=OrderItems.query.filter_by(product_type='Medication').all()
...
@app.route('/profitlossreport/',methods=['GET'])
@login_required
def IncomeExpense():
    # sales=OrderItems.query.filter_by(product_type='Medication').all()
    # service_sale=OrderItems.query.filter_by(product_type='Service').all()
    # productpurchase=[x.total_amount for x in PurchaseItems.query.all()]
    # saleslist=[x.total_amount for x in sales]
    # servicesale=[x.total_amount for x in service_sale]
    # totalsalesum=sum(saleslist)
    # totalservicessum=sum(servicesale)
...
def ProcessIncomeExpenseData():
    ...
    allexpenses=TrackExpense.query.all()
    def dictexpes():
        mainexpdic=[]
    ...
    data=[]
    data.append(incomepurchasedata)
    data.append(expensesresult)
